...
## 前提
- Python 3.12.9
- ルート直下にモデルファイル `models/kururi-orient-v1.pth` が配置されていること
...
uv run python cli/main.py predict --image <path_to_image> \
  [--device cpu|cuda:0] \
  [--checkpoint models/kururi-orient-v1.pth] \
  [--checkpoint-url https://example.com/kururi-orient-v1.pth] \
...
- デフォルトURLは `https://github.com/OWNER/REPO/releases/download/v0.1.0/kururi-orient-v1.pth` になっているため、公開時に実際のRelease URLへ差し替えてください。
